...
export default function StressMeter({ stressScore, label = true, size = 'md' }: StressMeterProps) {
    const score = Math.max(0, Math.min(100, stressScore));

    const getColor = () => {
        if (score <= 40) return { bar: '#10b981', bg: '#d1fae5', text: '#065f46', level: 'Low' };
        if (score <= 70) return { bar: '#f59e0b', bg: '#fef3c7', text: '#92400e', level: 'Moderate' };
        ...
        <div className="w-full">
            ...
            {label && (
                <div className="flex justify-between mt-1 text-xs text-gray-400">
                    <span>0</span>
                    <span className="font-semibold" style={{ color: colors.bar }}>{Math.round(score)}/100</span>
                    <span>100</span>
                </div>
            )}
        </div>
