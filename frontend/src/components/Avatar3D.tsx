...
interface Avatar3DProps {
    isTalking: boolean;
    emotion?: 'happy' | 'sad' | 'angry' | 'neutral' | 'anxious' | 'frustrated' | 'stressed' | 'lonely';
    userVolume?: number;
...
export default function Avatar3D({
    ...
    isListening = false,
    state = 'idle',
    modelPath = '/school_girl_sit_on_a_chair/scene.gltf'
}: Avatar3DProps) {
    const group = useRef<THREE.Group>(null);
    const { scene, animations, nodes } = useGLTF(modelPath) as any;
    ...
    useEffect(() => {
        ...
        const getBlinkInterval = () => {
            if (emotion === 'anxious') return 800 + Math.random() * 1200;   // fast, nervous
            if (emotion === 'sad') return 5000 + Math.random() * 4000;  // slow, heavy
            if (emotion === 'angry') return 2000 + Math.random() * 1500;  // tense
            return 3000 + Math.random() * 3000; // normal
        };
        const getBlinkDuration = () => {
            if (emotion === 'sad') return 250; // slow blink
            return 150;
        };
        const nextBlink = () => {
            setBlink(true);
    ...
    useEffect(() => {
        ...
        scene.traverse((node: any) => {
            ...
            if (node.isBone) {
                ...
                if (boneName.includes('head')) headRef.current = node;
                if (boneName.includes('spine2') || boneName.includes('chest')) chestRef.current = node;
                if (boneName.includes('shoulder')) {
                    if (boneName.includes('left')) node.userData.isLShoulder = true;
                    if (boneName.includes('right')) node.userData.isRShoulder = true;
                }
                return;
            }
    ...
    useFrame((frameState) => {
        ...
        Object.values(nodes).forEach((node: any) => {
            if (node.isBone) {
                ...
                if (node.userData.isLShoulder) node.rotation.z = Math.sin(t * 1.2) * 0.01;
                if (node.userData.isRShoulder) node.rotation.z = -Math.sin(t * 1.2) * 0.01;
            }
            ...
            if (node.morphTargetInfluences) {
                ...
                if (mouthIdx !== undefined) node.morphTargetInfluences[mouthIdx] = mouthOpen;
                ...
                if (smileIdx !== undefined) node.morphTargetInfluences[smileIdx] = smile;

                const frownIdx = dict['mouthFrown'] ?? dict['Frown'];
                if (frownIdx !== undefined) node.morphTargetInfluences[frownIdx] = frown;
    ...
    return (
        <group ref={group} dispose={null} scale={4.2} position={[0, -0.5, 0]}>
