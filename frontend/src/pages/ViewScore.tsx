...
export default function ViewScore() {
  ...
  const [radarScores, setRadarScores] = useState<{ focus: number; memory: number; decision: number; emotion: number } | null>(null);
      ...
      try {
        ...
        try {
          ...
          const anomalyRes = await fetch(
            `${import.meta.env.VITE_API_URL || 'http://localhost:8001/api'}/ml/anomaly-check?token=${token}`
          );
  ...
  const handleClearHistory = async () => {
    if (!window.confirm("Are you sure you want to clear all your stress and game history? This cannot be undone.")) return;
    try {
      setClearing(true);
  ...
  const chartData = useMemo(() =>
    stressHistory.slice().reverse().map((log, i) => ({
      session: i + 1,
    ...
    <div className="min-h-screen bg-amber-50 py-10 px-4">
      ...
      >
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-black text-stone-800">Your Progress</h1>
            <p className="text-stone-600 mt-1">ML-powered cognitive & stress analytics</p>
          </div>
          <motion.button
            onClick={() => navigate('/dashboard')}
            className="px-5 py-2.5 bg-white border border-amber-200 text-stone-700 rounded-xl font-medium text-sm hover:bg-amber-50 transition"
            whileTap={{ scale: 0.96 }}
          >
            ← Dashboard
          </motion.button>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            <motion.div
              className="w-12 h-12 border-4 border-amber-300 border-t-amber-500 rounded-full"
              animate={{ rotate: 360 }}
              transition={{ repeat: Infinity, duration: 0.8, ease: 'linear' }}
            />
            <p className="text-stone-500 mt-4 text-sm font-medium">Loading your analytics…</p>
          </div>
        ) : (
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  <motion.div
                    ...
                    whileHover={{ y: -3 }}
                  >
                    <p className="text-xs font-bold text-stone-500 uppercase tracking-wide mb-2">{m.label}</p>
                    <p className="text-3xl font-black" style={{ color: m.color }}>
                      {m.value.toFixed(0)}
                    </p>
                    <p className="text-xs font-medium text-stone-500">{m.suffix}</p>
                  </motion.div>
                ))}
              </div>
            )}
            ...
            {/* Tabs */}
            <div className="flex gap-2 p-1 bg-white rounded-2xl border border-amber-100">
              {tabs.map((tab) => (
                <button
                  key={tab.id}
            ...
            {activeTab === 'overview' && (
              ...
              >
                ...
                <div className="bg-white rounded-2xl border border-amber-100 p-6">
                  ...
                  <StressTrajectoryChart data={chartData} showCognitive={false} />
                  <p className="text-xs text-stone-400 mt-2 text-center">
                    Green zone (&lt;40) = low stress · Red zone (&gt;70) = high stress
                  </p>
                </div>
                ...
                {radarData.length > 0 && (
                  <div className="bg-white rounded-2xl border border-amber-100 p-6">
                    ...
                    <PerformanceRadarChart scores={radarData} />
                    <p className="text-xs font-medium text-stone-500 mt-2 text-center">
                      Average cognitive score per game type (higher = better)
                    </p>
                  </div>
                )}
              </motion.div>
            ...
            {activeTab === 'games' && gameStats && (
              <>
                {Object.keys(gameStats).length === 0 ? (
                  <div className="text-center py-12 text-stone-500 bg-white rounded-2xl border border-amber-100">
                    <p className="text-4xl mb-3">🧩</p>
                    <p className="font-semibold text-stone-700">No game statistics available yet!</p>
                    <p className="text-sm mt-1">Play the cognitive games to start tracking your individual performance metrics.</p>
                  </div>
                ) : (
                  <motion.div
                    ...
                    {Object.entries(gameStats).map(([gameName, stats]) => (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-4">
                          <span className="text-xl">{GAME_ICONS[gameName] || '🎮'}</span>
                        ...
                        <div className="space-y-2.5">
                          {[
                            ...
                            <div key={label} className="flex justify-between items-center text-sm">
                              ...
                              <span className="font-semibold text-stone-700">{value} <span className="font-medium text-stone-500 text-xs">{unit}</span></span>
                            </div>
                          ))}
                        </div>
                        {/* Mini progress bar */}
            ...
            {activeTab === 'history' && stressHistory.length > 0 && (
              <motion.div
                className="bg-white rounded-2xl border border-amber-100 overflow-hidden"
                initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              >
                <div className="flex items-center justify-between py-4 px-5 border-b border-amber-100 bg-amber-50">
                  <h3 className="font-bold text-stone-800 text-sm">Session Logs</h3>
                  <button
                ...
                <div className="grid grid-cols-3 py-3 px-5 border-b border-amber-100 bg-amber-50/50">
                  <span className="text-xs font-bold text-stone-600 uppercase tracking-wide">Date</span>
                  <span className="text-xs font-bold text-stone-600 uppercase tracking-wide text-center">Score</span>
                  <span className="text-xs font-bold text-stone-600 uppercase tracking-wide text-right">Level</span>
                </div>
                <div className="divide-y divide-amber-50 max-h-[420px] overflow-y-auto">
                  {stressHistory.map((log, idx) => {
                    const cc = LEVEL_COLORS[log.stress_level] || LEVEL_COLORS.moderate;
                    return (
                      <motion.div
                        key={idx}
                        className="grid grid-cols-3 py-3 px-5 hover:bg-amber-50/50 transition-colors"
                      ...
                      >
                        ...
                        <span className="text-sm font-bold text-stone-800 text-center">{log.stress_score.toFixed(0)}/100</span>
                        <div className="flex justify-end">
                          <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold text-white ${cc.badge}`}>
                            {log.stress_level}
                          </span>
                        </div>
                      </motion.div>
            ...
            {stressHistory.length === 0 && !loading && (
              <div className="text-center py-12 text-stone-400 bg-white rounded-2xl border border-amber-100">
                <p className="text-4xl mb-3">🎮</p>
                <p className="font-semibold">No data yet — play some games!</p>
                <p className="text-sm mt-1">Your analytics will appear here after your first session.</p>
                <motion.button
                  onClick={() => navigate('/games')}
                  className="mt-4 px-6 py-2.5 bg-amber-400 text-white rounded-xl font-bold text-sm"
